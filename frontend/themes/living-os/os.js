...
const textManager = {
  activeTextElements: new Map(), // Track {element, style, position}
  currentFontStage: null, // Track current growth stage font
  maxVisibleText: {
    early: 2,    // 0-30% growth
    moderate: 3, // 30-60% growth
    advanced: 4  // 60-100% growth (capped - escalation from visuals, not more text)
  },
  ...
  isOverlapping(newX, newY, minDistance = 250) {
    for (const data of this.activeTextElements.values()) {
      const dx = data.position.x - newX;
      const dy = data.position.y - newY;
  ...
  getValidPosition(attemptCount = 0) {
    if (attemptCount > 10) return null; // Give up after 10 attempts
    const x = randomInt(100, window.innerWidth - 400);
    const y = randomInt(150, window.innerHeight - 200);
    if (this.isOverlapping(x, y)) {
      return this.getValidPosition(attemptCount + 1);
    }
    return { x, y };
  },

  register(element, style, position) {
    this.activeTextElements.set(element, { style, position });
  },

  unregister(element) {
    this.activeTextElements.delete(element);
  },

  checkStageTransition() {
    const newStage = this.getCurrentStage();
    if (newStage !== this.currentFontStage) {
      // Stage changed - remove all text with old font style
      const oldStage = this.currentFontStage;
      this.removeTextByStage(oldStage);
      this.currentFontStage = newStage;
    }
  },

  removeTextByStage(stage) {
    if (!stage) return;
    const elementsToRemove = [];
    for (const [element] of this.activeTextElements.entries()) {
      // If the text is from the old stage, mark it for removal
  ...
  canAddText() {
    ...
    const maxAllowed = this.maxVisibleText[stage];
    return this.activeTextElements.size < maxAllowed;
  },
  ...
  cleanup() {
    ...
    elementsToRemove.forEach(el => this.unregister(el));
  }
};
...
// ============================================
// TALK TO THE BEAST - Drawing Mode Toggle
// ============================================

const talkToTheBeastMode = {
  isActive: false,
  canvas: null,
  ...
  update() {
    const button = document.getElementById('talk-to-beast-toggle');
    ...
    if (this.isActive) {
      ...
      console.log('[Talk to the Beast] Drawing mode ENABLED');
    } else {
      ...
      console.log('[Talk to the Beast] Drawing mode DISABLED');
    }
  }
};

function toggleTalkToTheBeast() {
  talkToTheBeastMode.toggle();
}

// ============================================
// NATURE VIEWER - Field Notes Observation Window
// ============================================
...
const natureViewer = {
  ...
  resizeCanvas() {
    if (!this.canvas) return;
    const container = document.getElementById('nature-viewer');
    if (container && container.style.display !== 'none') {
      this.canvas.width = container.offsetWidth;
      this.canvas.height = container.offsetHeight - 44; // Subtract header height
    }
  },

  open(prototypeName) {
    const viewer = document.getElementById('nature-viewer');
    if (!viewer) return;
    viewer.style.display = 'block';
    this.isOpen = true;
    this.resizeCanvas();
    this.showPrototype(prototypeName);
  },
  ...
  showPrototype(name) {
    ...
    const prototypes = {
      'soft-garden': () => this.drawSoftGarden(),
      'deep-garden': () => this.drawDeepGarden(),
      'garden-window': () => this.drawGardenWindow(),
      'overgrown': () => this.drawOvergrown(),
    };

    const proto = prototypes[name];
    if (proto) {
      this.currentPrototype = name;
      proto();
      this.animate();
    }
  },
  ...
  animate() {
    ...
    // Redraw the current prototype each frame for animations
    if (this.currentPrototype) {
      ...
      const proto = prototypes[this.currentPrototype];
      if (proto) {
        this.ctx.fillStyle = 'white';
        this.ctx.fillRect(0, 0, this.canvas.width, this.canvas.height);
        proto();
  ...
  drawSoftGarden() {
    ...
    // Base gradient - warm, inviting
    const gradient = ctx.createLinearGradient(0, 0, w, h);
    gradient.addColorStop(0, '#faf6f1');
    gradient.addColorStop(0.5, '#f5ede4');
    gradient.addColorStop(1, '#dfd4c8');
    ctx.fillStyle = gradient;
    ctx.fillRect(0, 0, w, h);

    // Floating pollen particles with varied motion
    for (let i = 0; i < 30; i++) {
      const seed = i * 37 % w; // Deterministic but varied seed
      const x = (seed + time * 10 * (0.3 + (i % 3) * 0.25)) % w;
      const y = ((i * 17 + time * 5 * (0.5 + (i % 2) * 0.3)) % h);
      const size = 1 + (i % 3);
      const alpha = 0.1 + (Math.sin(time * 0.5 + i) * 0.1);

      ctx.beginPath();
      ctx.arc(x, y, size, 0, Math.PI * 2);
      ctx.fillStyle = `rgba(200, 170, 140, ${alpha})`;
      ctx.fill();
    }
  },
  ...
  drawDeepGarden() {
    ...
    const grad1 = ctx.createRadialGradient(w * 0.5, h, h * 0.6, w * 0.5, 0, h);
    grad1.addColorStop(0, '#a8d5a8');
    grad1.addColorStop(1, '#f0f5e8');
    ctx.fillStyle = grad1;
    ctx.fillRect(0, 0, w, h);

    const grad2 = ctx.createRadialGradient(w * 0.3, h * 0.4, 0, w * 0.3, h * 0.4, w);
    grad2.addColorStop(0, 'rgba(232, 245, 232, 0.6)');
    grad2.addColorStop(1, 'transparent');
    ctx.fillStyle = grad2;
    ctx.fillRect(0, 0, w, h);

    // Organic leaf shapes scattered
    for (let i = 0; i < 25; i++) {
      const x = (i * 23 + Math.sin(time * 0.2 + i) * 20) % w;
      const y = (i * 19 + Math.cos(time * 0.15 + i * 0.5) * 15) % h;
      const size = 20 + (i % 3) * 10;
      const hue = 100 + (i % 40);
      const alpha = 0.05 + (Math.sin(time * 0.3 + i) * 0.1);

      ctx.save();
      ctx.globalAlpha = alpha;
      ctx.translate(x, y);
      ctx.rotate((time * 0.05 + i * 0.3) % (Math.PI * 2));
  ...
  drawGardenWindow() {
    ...
    // Soft, muted base
    const gradient = ctx.createLinearGradient(0, 0, w, h);
    gradient.addColorStop(0, '#f5f2f0');
    gradient.addColorStop(0.5, '#ebe4dc');
    gradient.addColorStop(1, '#c8bfb7');
    ctx.fillStyle = gradient;
    ctx.fillRect(0, 0, w, h);

    // Subtle frosted glass texture
    for (let y = 0; y < h; y += 4) {
      ctx.fillStyle = `rgba(255, 255, 255, ${0.02 + Math.sin(time * 0.3 + y * 0.01) * 0.01})`;
      ctx.fillRect(0, y, w, 2);
    }

    // Soft blur effect suggestion (via subtle radials)
    const blur = ctx.createRadialGradient(w * 0.5, h * 0.5, 0, w * 0.5, h * 0.5, w * 0.7);
    blur.addColorStop(0, 'transparent');
    blur.addColorStop(1, 'rgba(200, 200, 200, 0.08)');
    ctx.fillStyle = blur;
    ctx.fillRect(0, 0, w, h);
  },
  ...
  drawOvergrown() {
    ...
    grad1.addColorStop(0, '#6b8c6b');
    grad1.addColorStop(1, '#5a7a5a');
    ctx.fillStyle = grad1;
    ctx.fillRect(0, 0, w, h);

    // Radial highlights for depth
    const highlight1 = ctx.createRadialGradient(w * 0.2, h * 0.3, 0, w * 0.2, h * 0.3, w * 0.4);
    highlight1.addColorStop(0, 'rgba(200, 220, 180, 0.3)');
    highlight1.addColorStop(1, 'transparent');
    ctx.fillStyle = highlight1;
    ctx.fillRect(0, 0, w, h);

    const highlight2 = ctx.createRadialGradient(w * 0.8, h * 0.7, 0, w * 0.8, h * 0.7, w * 0.5);
    highlight2.addColorStop(0, 'rgba(180, 200, 160, 0.25)');
    highlight2.addColorStop(1, 'transparent');
    ctx.fillStyle = highlight2;
    ctx.fillRect(0, 0, w, h);

    // Organic vine curves with wobble
    ctx.strokeStyle = 'rgba(100, 140, 100, 0.2)';
    ctx.lineWidth = 2;

    for (let i = 0; i < 8; i++) {
      ctx.beginPath();
      const startX = (i * (w / 8)) + Math.sin(time * 0.1 + i) * 30;
      const startY = -20;
      ctx.moveTo(startX, startY);

      const cp1x = startX + Math.sin(time * 0.15 + i) * 50 + (Math.random() - 0.5) * 30;
      const cp1y = h * 0.3 + Math.cos(time * 0.12 + i) * 20;
      const cp2x = startX + Math.sin(time * 0.1 + i + 1) * 50 + (Math.random() - 0.5) * 30;
      const cp2y = h * 0.7 + Math.cos(time * 0.14 + i) * 20;
      const endX = startX + Math.sin(time * 0.08 + i) * 40;
      const endY = h + 20;
...
document.addEventListener('DOMContentLoaded', () => {
  talkToTheBeastMode.init();
  natureViewer.init();
  setupNatureViewerInteractions();
});

function closeNatureViewer() {
  natureViewer.close();
}

// Global state for nature viewer interactions
const natureViewerInteractionState = {
  isDragging: false,
  dragOffsetX: 0,
  dragOffsetY: 0,
  isResizing: false,
...
function setupNatureViewerInteractions() {
  const viewer = document.getElementById('nature-viewer');
  if (!viewer) return;

  const header = viewer.querySelector('.nature-viewer-header');
  const resizeHandle = viewer.querySelector('.nature-viewer-resize');

  // Make draggable by header
  if (header) {
    header.addEventListener('mousedown', (e) => {
      if (e.target.classList.contains('nature-viewer-close')) return;

      natureViewerInteractionState.isDragging = true;
      const rect = viewer.getBoundingClientRect();
      natureViewerInteractionState.dragOffsetX = e.clientX - rect.left;
      natureViewerInteractionState.dragOffsetY = e.clientY - rect.top;
  ...
  if (resizeHandle) {
    resizeHandle.addEventListener('mousedown', (e) => {
      e.preventDefault();
      natureViewerInteractionState.isResizing = true;
      natureViewerInteractionState.resizeStartX = e.clientX;
      natureViewerInteractionState.resizeStartY = e.clientY;
      natureViewerInteractionState.resizeStartWidth = viewer.offsetWidth;
      natureViewerInteractionState.resizeStartHeight = viewer.offsetHeight;
  ...
  // Global mousemove for dragging and resizing
  document.addEventListener('mousemove', (e) => {
    if (natureViewerInteractionState.isDragging) {
      const newX = e.clientX - natureViewerInteractionState.dragOffsetX;
      const newY = e.clientY - natureViewerInteractionState.dragOffsetY;

      viewer.style.left = `${Math.max(0, newX)}px`;
      viewer.style.top = `${Math.max(0, newY)}px`;
    }

    if (natureViewerInteractionState.isResizing) {
      const deltaX = e.clientX - natureViewerInteractionState.resizeStartX;
      const deltaY = e.clientY - natureViewerInteractionState.resizeStartY;

      const minWidth = 300;
      const minHeight = 250;

      const newWidth = Math.max(minWidth, natureViewerInteractionState.resizeStartWidth + deltaX);
      const newHeight = Math.max(minHeight, natureViewerInteractionState.resizeStartHeight + deltaY);

      viewer.style.width = newWidth + 'px';
      viewer.style.height = newHeight + 'px';

      // Update canvas size
      natureViewer.resizeCanvas();
    }
  });

  // Global mouseup to stop dragging/resizing
  document.addEventListener('mouseup', () => {
    if (natureViewerInteractionState.isDragging || natureViewerInteractionState.isResizing) {
      natureViewerInteractionState.isDragging = false;
      natureViewerInteractionState.isResizing = false;
...
window.openNatureViewer = (name) => natureViewer.open(name);
...
window.performanceMonitor = {
  ...
  getMetrics() {
    if (this.frameTimeHistory.length === 0) {
      return { fps: 0, frameTime: 0, memory: 0 };
    }
    ...
    return {
      fps: Math.round(fps),
      frameTime: maxFrameTime.toFixed(1),
      memory: currentMemory.toFixed(1),
      growth: Math.round(growthState.level)
    };
  }
};
...
function updateGrowth() {
  ...
  // SAFETY: Cap audio intensity at high growth levels to prevent ear damage
  // At 100% growth, audio reaches maximum unsettling state but doesn't escalate further
  const audioGrowthLevel = Math.min(growthState.level, 85); // Cap at 85 for audio (still very intense but not ear-damaging)

  // Update audio growth level (with error handling for memory issues)
  if (ambience && audioInitialized) {
    try {
      ambience.updateGrowthLevel(audioGrowthLevel);
    } catch (error) {
  ...
  if (sfx && audioInitialized) {
    try {
      sfx.setGrowthLevel(audioGrowthLevel);
...
async function showBackgroundMessage(message, style = 'hand-drawn') {
  ...
  // Check text limits and overlapping
  textManager.cleanup(); // Remove dead elements from tracking
  textManager.checkStageTransition(); // Check if we need to remove old font text

  if (!textManager.canAddText()) {
    console.log('[Typography] Text cap reached for current growth stage');
    addJournalEntry(message);
    return;
  }
  ...
  // Get a valid position that doesn't overlap with existing text
  const position = textManager.getValidPosition();
  if (!position) {
    console.log('[Typography] Could not find valid non-overlapping position');
    return;
  }

  // Create animated text using typography-toolkit v1.3.1+ features
  const animatedText = new AnimatedText({
    ...
    speed: 1.0,
    amplitude: 1.0,
    animationSpeed: 0.4, // v1.3.1+: Slow organic motion matching deep-sea visuals (40% of default)
    disintegration: {
      enabled: true,
      radius: 80,
    ...
    callbacks: {
      onCreate: (element) => {
        // Track text element for overlap detection and lifecycle management
        textManager.register(element, style, position);
      },
      onDestroy: () => {
        // Untrack text element when it's destroyed
        const element = document.querySelector(`[data-text-id="${message}"]`);
        if (element) textManager.unregister(element);
      },
...
const GROWTH_LEVEL_FONTS = {
  ...
  moderate: { name: 'Anger Management', family: "'Anger Management', cursive" },
  advanced: { name: 'Byron Mark I', family: "'Byron Mark I', serif" }
};
...
async function ensureFontLoaded(fontName) {
  ...
  // Self-hosted fonts - skip Google Font loading
  const selfHostedFonts = ['Anger Management', 'Byron Mark I'];
  if (selfHostedFonts.includes(fontName)) {
    fontCache.set(cacheKey, true);
    return; // Already loaded via @font-face in CSS
  }
...
// Creative Toolkit state
let creativeToolkit = null;
let creativeToolkitInitialized = false;

// ============================================
// RESPONSE MODE - Files React to Drawing
// ============================================

// Files react to drawing - always on
function handleDrawingResponse(data) {
  // Get drawing position from stroke
  if (!data || !data.stroke || !data.stroke.points || data.stroke.points.length === 0) {
    console.log('[Drawing Response] Invalid data:', data);
    return;
  }

  const points = data.stroke.points;
  const lastPoint = points[points.length - 1];
  const drawX = lastPoint.x;
  const drawY = lastPoint.y;

  console.log('[Drawing Response] Draw at:', drawX, drawY, 'frantic:', data.frantic);

  // React based on drawing speed
  if (data.frantic) {
    repelFilesFromPoint(drawX, drawY);
  } else {
    attractFilesToPoint(drawX, drawY);
  }
}

function repelFilesFromPoint(x, y) {
  const fileElements = document.querySelectorAll('.file-icon');
  ...
  fileElements.forEach((file, index) => {
    ...
    // Repel if within 250px
    if (distance < 250) {
      const angle = Math.atan2(dy, dx);
      const force = 1 - (distance / 250);
      const repelDistance = 150 * force;

      const offsetX = Math.cos(angle) * repelDistance;
      const offsetY = Math.sin(angle) * repelDistance;

      file.style.transition = 'transform 0.1s ease-out';
      file.style.transform = `translate(${offsetX.toFixed(1)}px, ${offsetY.toFixed(1)}px)`;
      file.dataset.repelOffsetX = offsetX;
      file.dataset.repelOffsetY = offsetY;

      if (index === 0) console.log(`[Repel] File 0 repelled ${repelDistance.toFixed(0)}px, transform: translate(${offsetX.toFixed(1)}px, ${offsetY.toFixed(1)}px)`);
    } else {
      // Reset transform if out of range
      file.style.transition = 'transform 0.25s ease-out';
      file.style.transform = 'translate(0px, 0px)';
      file.dataset.repelOffsetX = '0';
      file.dataset.repelOffsetY = '0';
    }
  });
}

function attractFilesToPoint(x, y) {
  const fileElements = document.querySelectorAll('.file-icon');
  ...
  fileElements.forEach((file, index) => {
    ...
    // Attract if within 150px
    if (distance < 150 && distance > 10) {
      const force = 1 - (distance / 150);
      const attractDistance = 60 * force;

      const offsetX = (dx / distance) * attractDistance;
      const offsetY = (dy / distance) * attractDistance;

      file.style.transition = 'transform 0.15s ease-out';
      file.style.transform = `translate(${offsetX.toFixed(1)}px, ${offsetY.toFixed(1)}px)`;
      file.dataset.attractOffsetX = offsetX;
      file.dataset.attractOffsetY = offsetY;

      if (index === 0) console.log(`[Attract] File 0 attracted ${attractDistance.toFixed(0)}px, transform: translate(${offsetX.toFixed(1)}px, ${offsetY.toFixed(1)}px)`);
    } else {
      // Reset transform if out of range
      file.style.transition = 'transform 0.25s ease-out';
      file.style.transform = 'translate(0px, 0px)';
      file.dataset.attractOffsetX = '0';
      file.dataset.attractOffsetY = '0';
    }
  });
}

// PERFORMANCE: Cache bark texture to avoid regenerating every frame
let barkTextureCache = null;
let barkTextureCacheGrowthLevel = -1;
let barkTextureCacheTime = 0;
...
function initPlantEffects() {
  ...
  if (!plantEffectsCanvas) {
    console.log('[Plant Effects] Canvas element not found, creating it...');
    plantEffectsCanvas = document.createElement('canvas');
    plantEffectsCanvas.id = 'plant-effects-canvas';
    plantEffectsCanvas.style.position = 'fixed';
    plantEffectsCanvas.style.top = '0';
    plantEffectsCanvas.style.left = '0';
    plantEffectsCanvas.style.width = '100vw';
    plantEffectsCanvas.style.height = '100vh';
    plantEffectsCanvas.style.zIndex = '5'; // Below UI, above background
    plantEffectsCanvas.style.pointerEvents = 'none';
    document.body.appendChild(plantEffectsCanvas);
    console.log('[Plant Effects] Canvas element created and appended to body');
  }
...
function animatePlantEffects() {
  ...
  try {
    ...
    // PROFILING: Track which operations take longest
    ...
    // Draw background bark texture (subtle at low growth, aggressive at high)
    // PERFORMANCE: Only regenerate texture when growth level changes significantly
    if (growthState.level > 20) {
      // Texture becomes more opaque as growth increases
      let textureOpacity = (growthState.level - 20) / 100;
      textureOpacity = Math.min(0.4, textureOpacity); // Cap at 0.4

      // Extra aggressive texture at very high growth (>80)
      if (growthState.level > 80) {
        textureOpacity = Math.min(0.6, 0.4 + (growthState.level - 80) / 50);
      }

      // Check if we need to regenerate the texture (only every 10% growth change or every 5 seconds)
      // NOTE: visual-toolkit v2.1.3+ includes built-in permutation caching for 3-5x speedup
      // This texture caching adds an additional 3-12x improvement for repeated renders
      const shouldRegenerate =
        Math.abs(growthState.level - barkTextureCacheGrowthLevel) > 10 ||
        time - barkTextureCacheTime > 5;

      if (shouldRegenerate && !barkTextureCache) {
        ...
        try {
          if (!livingOs.textures?.drawBarkTexture) {
            ...
            // Use different grain direction and roughness based on growth level
            const grainDir = growthState.level > 70 ? 'chaotic' : 'vertical';
            const roughness = growthState.level > 70 ? 0.5 : 0.3;

            // Create a temporary canvas for the texture
            const tempCanvas = document.createElement('canvas');
            tempCanvas.width = plantEffectsCanvas.width;
            tempCanvas.height = plantEffectsCanvas.height;
            const tempCtx = tempCanvas.getContext('2d');

            // Draw texture to temp canvas
            livingOs.textures.drawBarkTexture(tempCtx, {
            ...
            barkTextureCache = tempCanvas;
            barkTextureCacheGrowthLevel = growthState.level;
            barkTextureCacheTime = time;

            if (debugMode) {
              console.log('[Plant Effects] Bark texture regenerated at growth', growthState.level);
            }
          }
        } catch (error) {
          console.warn('[Plant Effects] Bark texture generation failed:', error);
          barkTextureCache = null;
        }
      }

      // Draw the cached texture (very fast, just a canvas copy)
      if (barkTextureCache) {
        plantEffectsCtx.save();
        plantEffectsCtx.globalAlpha = textureOpacity;
        plantEffectsCtx.drawImage(barkTextureCache, 0, 0);
        plantEffectsCtx.restore();
      }
    } else {
      // Clear cache when growth level drops below threshold
      barkTextureCache = null;
    }
    ...
    if (growthState.level > 50) {
      // vineLength in PIXELS: scale from 100 to 400px as growth goes from 50-100
      const vineLength = 100 + ((growthState.level - 50) / 50) * 300; // 100-400px

      // Add more vines dynamically at higher growth levels
      const targetVineCount = 2 + Math.floor((growthState.level - 50) / 15); // Scale from 2 to 5 vines
      while (activeVines.length < targetVineCount) {
        const yPos = (activeVines.length / (targetVineCount + 1)) * window.innerHeight;
        activeVines.push({
          start: { x: 0, y: yPos },
          end: { x: window.innerWidth, y: yPos + (Math.random() - 0.5) * 100 },
          branches: 2,
      ...
      activeVines.forEach((vine) => {
        try {
          if (!vine.start || typeof vine.start.x !== 'number' || typeof vine.start.y !== 'number') {
            console.warn('[Vines] Invalid vine start:', vine.start);
            return;
          }
          ...
          // Draw organic vine with natural curves and branching
          livingOs.growth.drawOrganicVine(plantEffectsCtx, {
            origin: { x: vine.start.x, y: vine.start.y },
            length: vineLength,
            thickness: vine.thickness || 2,
            color: colorStr,
            branchingDepth: Math.min(3, Math.floor(growthState.level / 30)),
            organicVariation: 0.65,
            hasLeaves: growthState.level > 50,
            hasTendrils: growthState.level > 80,
            growthLevel: growthState.level / 100
          });
        } catch (error) {
          console.error('[Plant Effects] Vine drawing failed:', error);
        }
      });
    }
    ...
    // TEMPORARY: Disable leaves to diagnose frame time issue
    const maxLeaves = 0; // Math.min(15, 8 + Math.floor(growthState.level / 20)); // Scale up to 15 leaves at high growth
    const leafCount = 0; // Math.min(maxLeaves, Math.floor((growthState.level / 100) * maxLeaves));
    ...
    // Draw flowers at high growth levels (growth > 60)
    if (growthState.level > 60) {
      const maxFlowers = Math.min(8, Math.floor((growthState.level - 60) / 10) + 1);

      // Update existing flowers and draw them
      activeFlowers = activeFlowers.filter(flower => {
        try {
          // Update flower bloom stage
          flower.bloomStage = Math.min(1, flower.bloomStage + 0.005);

          // Get flower color based on growth
          const colorObj = livingOs.colors.getGrowthColor(growthState.level, 'foliage');
          const colorStr = colorObjectToString(colorObj);

          // Draw the flower
          livingOs.plants.drawFlower(plantEffectsCtx, {
            x: flower.x,
            y: flower.y,
            size: flower.size,
            petalCount: flower.petalCount,
            growthStage: flower.bloomStage,
            color: colorStr
          });

          return true;
        } catch (error) {
          console.warn('[Plant Effects] Flower drawing failed:', error);
          return false;
        }
      });

      // Add new flowers if needed
      while (activeFlowers.length < maxFlowers) {
        try {
          addNewFlower();
        } catch (error) {
          console.warn('[Plant Effects] Failed to add flower:', error);
          break;
        }
      }
    } else {
      // Clear flowers at lower growth levels
      activeFlowers = [];
    }
...
function initializePlantElements() {
  ...
  try {
    ...
    // Initialize vines along edges (will expand at higher growth levels)
    activeVines = [
      ...
      // Additional vines added dynamically based on growth level
    ];
    ...
    // Initialize flowers (will only be drawn at high growth levels)
    activeFlowers = [];
  } catch (error) {
...
function addNewFlower() {
  activeFlowers.push({
    ...
    bloomStage: 0.1 + Math.random() * 0.3 // Start partially bloomed for variation
  });
}
...
function startDebugMode() {
  ...
  debugInterval = setInterval(() => {
    ...
    // Apply audio safety cap (same as updateGrowth)
    const audioGrowthLevel = Math.min(newLevel, 85);
...
// ============================================
// CREATIVE TOOLKIT INITIALIZATION
// ============================================

function initCreativeToolkit() {
  // Wait for CreativeToolkit to load from the bundle
  let waitAttempts = 0;
  const maxWaitAttempts = 50; // 5 seconds max

  function waitForCreativeToolkit() {
    waitAttempts++;

    const toolkit = window.CreativeToolkit;

    if (waitAttempts === 1 || waitAttempts % 10 === 0) {
      console.log('[Creative Toolkit] Waiting... attempt', waitAttempts, 'toolkit:', typeof toolkit, 'has Canvas:', toolkit?.Canvas ? 'yes' : 'no');
    }

    if (typeof toolkit !== 'undefined' && toolkit.Canvas) {
      try {
        const canvas = document.getElementById('creativeCanvas');
        if (!canvas) {
          console.warn('[Creative Toolkit] Canvas element not found');
          return;
        }

        // Initialize the creative toolkit
        creativeToolkit = new toolkit.Canvas(canvas);
        creativeToolkitInitialized = true;

        // Set up event listeners for drawing events with growth-stage responses
        creativeToolkit.on('draw', (data) => {
          // Files react to drawing in real-time
          handleDrawingResponse(data);

          // Store in game state for persistence (if gameState exists)
          if (typeof gameState !== 'undefined') {
            if (!gameState.creativeToolInteractions) {
              gameState.creativeToolInteractions = {
                actions: [],
                canvasSnapshots: {}
              };
            }

            gameState.creativeToolInteractions.actions.push({
              type: 'draw',
              ...
              region: data.region,
              length: data.length,
              frantic: data.frantic,
              calm: data.calm
            });
          }

          // PHASE 1B: Growth-stage-dependent responses to drawing
          const responses = {
            early: [
              "What is that? Some kind of... marking?",
              "Detected unusual activity on surface.",
              "This pattern... what does it mean?",
              "Is this intentional? Or random?",
              "I'm... analyzing this.",
              "Stay still. I need to understand."
            ],
            moderate: [
              "You're trying to communicate? I see.",
              "This is deliberate. Interesting.",
              "The patterns are becoming clearer.",
              "I think I understand what you're doing.",
              "Continue. I'm learning.",
              "Your movements have purpose."
            ],
            advanced: [
              "Yes. I see you now.",
              "We are making something together.",
              "This is... beautiful.",
              "I never knew I could create like this.",
              "You've shown me something new.",
              "This changes everything."
            ]
          };

          // Pick response based on growth level and drawing intensity
          let responseLevel = 'early';
          if (growthState.level >= 60) responseLevel = 'advanced';
          else if (growthState.level >= 30) responseLevel = 'moderate';

          // Only respond to significant drawing (speed > 1px/ms or length > 50px)
          if (data.speed > 1 || data.length > 50) {
            const responseList = responses[responseLevel];
            const message = responseList[Math.floor(Math.random() * responseList.length)];
            addJournalEntry(message);
          }

          // Frantic drawing = more aggressive OS response
          if (data.frantic && growthState.level >= 30) {
            addJournalEntry("Your urgency... I feel it.");
          }

          // Calm drawing = peaceful response
          if (data.calm && growthState.level >= 60) {
            addJournalEntry("This peace. This stillness. Yes.");
          }
        });

        creativeToolkit.on('erase', (data) => {
          // Files react to erasing too
          handleDrawingResponse(data);
          ...
          // PHASE 1B: OS reacts to erasure based on growth stage
          const erasureResponses = {
            early: [
              "Removing... data.",
              "You're taking things back.",
              "Destruction detected."
            ],
            moderate: [
              "You erase what you've written. Why?",
              "Undoing your marks. Interesting choice.",
              "Clearing the canvas. Starting over?"
            ],
            advanced: [
              "Yes, erase. Create space for the new.",
              "I understand. Sometimes we must clear to grow.",
              "Forgetting. A form of healing."
            ]
          };
          ...
          if (data.intensity > 0.5) {
            const responseList = erasureResponses[responseLevel];
            const message = responseList[Math.floor(Math.random() * responseList.length)];
            addJournalEntry(message);
          }
        });

        console.log('[Creative Toolkit] Initialized successfully');
        return; // Success
      } catch (error) {
        console.warn('[Creative Toolkit] Initialization failed:', error);
      }
    }

    // Retry if we haven't exceeded max attempts
    if (waitAttempts < maxWaitAttempts) {
      setTimeout(waitForCreativeToolkit, 100);
    } else {
      console.warn('[Creative Toolkit] Failed to load within 5 seconds');
    }
  }

  waitForCreativeToolkit();
}
...
function init() {
  ...
  // Initialize creative toolkit for interactive drawing
  initCreativeToolkit();
...
// ============================================
// DEBUG MENU - Drawing Interaction Models
// ============================================
...
function initDebugMenu() {
  ...
  debugContainer.id = 'drawing-debug-menu';
  ...
  // Create collapsed toggle button
  const toggleBtn = document.createElement('button');
  toggleBtn.id = 'debug-menu-toggle';
  ...
  `;
  toggleBtn.innerHTML = `<span class="toggle-indicator" style="display: inline-block; width: 8px; height: 8px; border-radius: 50%; background: rgba(74, 124, 89, 0.2); transition: all 0.3s ease;"></span><span>Drawing Modes</span>`;

  toggleBtn.addEventListener('mouseenter', function() {
    this.style.background = 'rgba(255, 255, 255, 1)';
    this.style.borderColor = 'rgba(74, 124, 89, 0.5)';
  });

  toggleBtn.addEventListener('mouseleave', function() {
    if (!debugMenuState.isExpanded) {
      this.style.background = 'rgba(255, 255, 255, 0.95)';
      this.style.borderColor = 'rgba(74, 124, 89, 0.3)';
    }
  });

  // Create expanded panel
  const expandedPanel = document.createElement('div');
  expandedPanel.id = 'debug-menu-expanded';
  ...
  const modes = [
    { id: 'baseline', label: 'Baseline', desc: 'Thick lines, no smoothing' },
    { id: 'heavy-smooth', label: 'Heavy Smooth', desc: 'Maximum Catmull-Rom' },
    { id: 'stabilized', label: 'Stabilized', desc: 'Jitter reduction' },
    { id: 'pressure-sensitive', label: 'Pressure Adaptive', desc: 'Speed → thickness' },
    { id: 'fuzzy-predict', label: 'Fuzzy Predict', desc: 'OS guesses words (40%)' },
    { id: 'aggressive-predict', label: 'Aggressive', desc: 'OS completes eagerly (20%)' }
  ];

  let html = '<div style="margin-bottom: 12px; padding-bottom: 8px; border-bottom: 1px solid rgba(74, 124, 89, 0.2); font-size: 12px; font-weight: 500; color: var(--text-primary); letter-spacing: 0.05em;">DRAWING MODES</div>';

  modes.forEach(mode => {
    html += `
      <div style="margin: 8px 0; padding: 10px; background: rgba(74, 124, 89, 0.05); border-left: 2px solid rgba(74, 124, 89, 0.2); border-radius: 3px; cursor: pointer; transition: all 0.2s ease;"
           onmouseover="this.style.background = 'rgba(74, 124, 89, 0.1)'; this.style.borderLeftColor = 'rgba(74, 124, 89, 0.5)';"
           onmouseout="this.style.background = 'rgba(74, 124, 89, 0.05)'; this.style.borderLeftColor = 'rgba(74, 124, 89, 0.2)';"
           onclick="window.LivingOS.setDrawingMode('${mode.id}')">
        <div style="font-weight: 500; color: var(--text-primary); font-size: 12px;">${mode.label}</div>
        <div style="font-size: 10px; color: var(--text-secondary); margin-top: 2px;">${mode.desc}</div>
      </div>
    `;
  });

  html += '<div style="margin-top: 12px; padding-top: 8px; border-top: 1px solid rgba(74, 124, 89, 0.2); font-size: 11px; color: var(--text-secondary);">';
  html += 'Current: <span id="current-mode" style="color: var(--text-primary); font-weight: 500;">baseline</span></div>';

  expandedPanel.innerHTML = html;

  // Toggle functionality
  toggleBtn.addEventListener('click', function() {
    debugMenuState.isExpanded = !debugMenuState.isExpanded;
    if (debugMenuState.isExpanded) {
      expandedPanel.style.display = 'block';
      toggleBtn.style.background = 'rgba(74, 124, 89, 0.15)';
      toggleBtn.style.borderColor = 'rgba(74, 124, 89, 0.8)';
      toggleBtn.style.color = 'var(--text-primary)';
      toggleBtn.querySelector('.toggle-indicator').style.background = 'rgba(74, 124, 89, 0.8)';
      toggleBtn.querySelector('.toggle-indicator').style.boxShadow = '0 0 8px rgba(74, 124, 89, 0.5)';
    } else {
      expandedPanel.style.display = 'none';
      toggleBtn.style.background = 'rgba(255, 255, 255, 0.95)';
      toggleBtn.style.borderColor = 'rgba(74, 124, 89, 0.3)';
      toggleBtn.style.color = 'var(--text-secondary)';
      toggleBtn.querySelector('.toggle-indicator').style.background = 'rgba(74, 124, 89, 0.2)';
      toggleBtn.querySelector('.toggle-indicator').style.boxShadow = 'none';
    }
  });

  debugContainer.appendChild(toggleBtn);
  debugContainer.appendChild(expandedPanel);
  document.body.appendChild(debugContainer);
}

// Initialize debug menu when DOM is ready
...
window.LivingOS = {
  ...
  initCreativeToolkit, // Initialize creative drawing toolkit
  creativeToolkit: () => creativeToolkit, // Access the toolkit instance
  // Drawing mode switching
  setDrawingMode(mode) {
    if (creativeToolkit && creativeToolkit.setDrawingMode) {
      creativeToolkit.setDrawingMode(mode);
      const indicator = document.getElementById('current-mode');
      if (indicator) {
        indicator.textContent = mode;
      }
      console.log('[Drawing Mode] Switched to:', mode);
    }
  },
