 ...
 */
export function drift(time, primary = timing.verySlow, secondary = timing.slow) {
    return {
 ...
 */
export function bob(time, speed = timing.slow, height = 0.02) {
 ...
 */
export function pulse(time, speed = timing.medium, min = 0.8, max = 1.2) {
 ...
 */
export function waver(time, segmentIndex, speed = timing.slow) {
    return {
 ...
 */
export function createSeeker(canvasWidth, canvasHeight) {
    return {
 ...
 */
export function updateSeeker(seeker, lightX, lightY, lightSpeed, canvasWidth, canvasHeight) {
    ...
    // Wrap at edges
    if (seeker.x < -50)
        seeker.x = canvasWidth + 50;
    if (seeker.x > canvasWidth + 50)
        seeker.x = -50;
    if (seeker.y < -50)
        seeker.y = canvasHeight + 50;
    if (seeker.y > canvasHeight + 50)
        seeker.y = -50;
}
/**
 ...
 * });
 */
export function createSeekerSwarm(count, canvasWidth, canvasHeight, options = {}) {
    const { spawnBias = 'uniform', sizeRange = [2, 8], speedRange = [1, 4], hueRange = [180, 220], } = options;
    const seekers = [];
 ...
 */
export function updateSeekerWithDrift(seeker, lightX, lightY, lightSpeed, canvasWidth, canvasHeight, centerDrift = 0) {
 ...
 * See: docs/SCENE_FIRST_ARCHITECTURE.md
 */
export function updateSeekerSwarm(seekers, lightX, lightY, lightSpeed, canvasWidth, canvasHeight, centerDrift = 0) {
 ...
 */
export function drawBiolumGlow(ctx, x, y, size, hue, intensity, time) {
    // Optional pulse
 ...
 */
export function drawSeeker(ctx, seeker, lightRadius = 150, lightX, lightY, time) {
 ...
 * });
 */
export function drawSeekerSwarm(ctx, seekers, options = {}) {
    ...
    const sorted = [...seekers].sort((a, b) => {
        ...
        if (lightX !== undefined && lightY !== undefined) {
            ...
            if (distA < lightRadius)
                visA = Math.max(visA, (1 - distA / lightRadius) * 0.8);
            if (distB < lightRadius)
                visB = Math.max(visB, (1 - distB / lightRadius) * 0.8);
        }
        return visA - visB;
    });
 ...
 */
export function staggerDelay(index, baseDelay = 0.3) {
