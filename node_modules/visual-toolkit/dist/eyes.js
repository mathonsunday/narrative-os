 ...
 * ];
 */
export function createEye(options) {
    return {
        x: options.x,
        y: options.y,
        size: options.size ?? 8,
 ...
 * }
 */
export function updateEye(eye, lightX, lightY, deltaTime = 16) {
 ...
 * }
 */
export function drawEye(ctx, eye, options = {}) {
    const { showGlow = true, glowSize = 2.5, } = options;
    const currentSize = eye.size * eye.openness;
    if (currentSize < 0.5)
        return; // Don't draw if basically closed
 ...
 */
export function createEyes(positions) {
    return positions.map(createEye);
}
/**
 * Update all eyes in an array
 */
export function updateEyes(eyes, lightX, lightY, deltaTime = 16) {
    for (const eye of eyes) {
 ...
 */
export function drawEyes(ctx, eyes, options) {
    for (const eye of eyes) {
