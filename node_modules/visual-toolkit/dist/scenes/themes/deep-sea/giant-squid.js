...
export class GiantSquidScene extends BaseCanvasScene {
    constructor() {
        ...
        this.description = 'Encounter with massive cephalopod';
        this.defaultConfig = {
            intensity: 0.8,
            scale: 1,
            duration: Infinity
    ...
    drawMassiveTentacle(ctx, width, height) {
        ...
        for (let i = 1; i < points.length; i++) {
            ...
            if (p0.x < -100 && p1.x < -100)
                continue;
            if (p0.x > width + 100 && p1.x > width + 100)
                continue;
        ...
        for (let i = 4; i < points.length - 4; i += 6) {
            ...
            const angle = Math.atan2(dy, dx) + Math.PI / 2;
            ctx.beginPath();
            ctx.moveTo(p.x + Math.cos(angle) * p.thickness * 0.4, p.y + Math.sin(angle) * p.thickness * 0.4);
            ctx.lineTo(p.x - Math.cos(angle) * p.thickness * 0.4, p.y - Math.sin(angle) * p.thickness * 0.4);
            ctx.stroke();
        }
    }
    render(ctx, deltaTime) {
        ...
        if (cursor.isOver && cursor.x > 0 && cursor.x < width) {
            const lightGrad = ctx.createRadialGradient(cursor.x, cursor.y, 0, cursor.x, cursor.y, 180);
