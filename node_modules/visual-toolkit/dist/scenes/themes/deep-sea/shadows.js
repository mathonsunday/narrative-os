...
export class ShadowsScene extends BaseCanvasScene {
    constructor() {
        super(...arguments);
        this.name = 'Shadows';
        this.description = 'Jellyfish-like creatures drifting in the deep';
        this.defaultConfig = {
            intensity: 0.5,
            scale: 1,
            duration: Infinity
    ...
    render(ctx, deltaTime) {
        ...
        if (cursor.isOver) {
            const lightGrad = ctx.createRadialGradient(cursor.x, cursor.y, 0, cursor.x, cursor.y, 200);
        ...
        // Vignette edge darkening
        const vigGrad = ctx.createRadialGradient(width / 2, height / 2, height * 0.25, width / 2, height / 2, height * 0.85);
    ...
    calculateIllumination(shadowX, shadowY, cursor) {
        if (!cursor.isOver) {
            return 0;
        }
    ...
    updateAndDrawShadows(ctx, width, height) {
        const cursor = this.getCursorPos();
        // Update shadows
        ...
        for (const shadow of this.shadows) {
            ...
            const illumination = this.calculateIllumination(shadow.x, shadow.y, cursor);
            // Fill bell
            const baseAlpha = 0.8;
