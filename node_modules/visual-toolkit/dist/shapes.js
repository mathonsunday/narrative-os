 ...
 */
export function drawPanelLines(ctx, x, y, width, height, linePositions = [0.25, 0.75], // normalized positions
 ...
 */
export function drawGrille(ctx, x, y, width, height, lineCount = 4, color = '#555') {
    ctx.strokeStyle = color;
    ctx.lineWidth = 2;
 ...
 */
export function drawTether(ctx, startX, startY, endX, endY, time, wobbleAmount = 10, color = 'rgba(80, 80, 80, 0.6)') {
    ctx.strokeStyle = color;
    ctx.lineWidth = 3;
    ctx.setLineDash([8, 4]);
 ...
 */
export function drawLens(ctx, x, y, outerRadius, innerRadius, outerColor = '#222', innerColor = '#1a1a2e', glintColor = 'rgba(100, 150, 200, 0.5)') {
    // Outer ring
 ...
 */
export function drawLight(ctx, x, y, radius, isOn, time) {
    // Housing
    ctx.fillStyle = '#444';
    ctx.beginPath();
 ...
 */
export function drawTrackingEye(ctx, eyeX, eyeY, eyeSize, targetX, targetY, illumination = 0 // 0-1, how much light is on it
) {
    // Eye socket glow
 ...
 */
export function drawTentacle(ctx, segments, thickness, color = 'rgba(10, 10, 15, 0.9)') {
