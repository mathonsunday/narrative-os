 ...
 * });
 */
export function drawOrganicSurface(ctx, width, height, options = {}) {
    // Apply preset first, then override with explicit options
    const presetConfig = options.preset ? surfacePresets[options.preset] : {};
    const { type = 'fleshy', noiseScale = presetConfig.noiseScale ?? 0.003, showVeins = presetConfig.showVeins ?? true, veinCount = presetConfig.veinCount ?? 6, seed = 42, time = 0, lightX, lightY, lightRadius = 250, } = { ...presetConfig, ...options };
...
// ============================================
function drawMottledBase(ctx, width, height, palette, perm, noiseScale, time) {
    ...
    if (width > MAX_WIDTH || height > MAX_HEIGHT) {
        console.warn(`drawOrganicSurface: Dimensions ${width}x${height} exceed safe limit (${MAX_WIDTH}x${MAX_HEIGHT}). ` +
    ...
    if (actualPixels > MAX_PIXELS) {
        ...
        console.warn(`drawOrganicSurface: Pixel count ${actualPixels} exceeds limit ${MAX_PIXELS}. ` +
    ...
    try {
        ...
        if (error instanceof RangeError || error instanceof DOMException) {
            ...
            console.error(`drawOrganicSurface: Failed to allocate ${actualWidth}x${actualHeight} buffer. ` +
                `Falling back to ${fallbackWidth}x${fallbackHeight}.`, error);
...
// ============================================
function drawDepthLayer(ctx, width, height, palette, perm, noiseScale) {
...
function drawOrganicVeins(ctx, width, height, palette, perm, count, time) {
    for (let v = 0; v < count; v++) {
        ...
        drawOrganicVeinBranch(ctx, startX, startY, startAngle, 5 + Math.abs(simplex2D(v * 20, v * 20, perm)) * 4, // thickness 5-9
        0, perm, palette, time, width, height, v * 1000);
    }
}
function drawOrganicVeinBranch(ctx, startX, startY, angle, thickness, depth, perm, palette, time, width, height, seedOffset) {
    if (thickness < 1 || depth > 4)
        return;
    // Generate organic path using bezier curves
    const points = [{ x: startX, y: startY, thickness }];
    let currentX = startX;
    let currentY = startY;
    ...
    for (let i = 0; i < segmentCount; i++) {
        ...
        if (i > 2 && simplex2D(seedOffset + i * 25, depth * 200, perm) > 0.7 && depth < 3) {
            ...
            drawOrganicVeinBranch(ctx, currentX, currentY, branchAngle, currentThickness * 0.6, depth + 1, perm, palette, time, width, height, seedOffset + i * 1000 + depth * 10000);
        }
    }
...
// ============================================
function drawSubsurfaceDetail(ctx, width, height, palette, perm, scale) {
...
// ============================================
function drawFleshLightResponse(ctx, width, height, lightX, lightY, lightRadius, palette, perm) {
    // Subsurface scattering - light penetrates flesh, creates warm glow
    // Inner warm glow (light penetrating tissue)
    const scatterGrad = ctx.createRadialGradient(lightX, lightY, 0, lightX, lightY, lightRadius * 1.5);
    ...
    const highlightGrad = ctx.createRadialGradient(lightX, lightY, 0, lightX, lightY, lightRadius * 0.5);
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (veinAlpha > 0.05) {
            ...
            ctx.quadraticCurveTo(vx + Math.cos(cpAngle) * cpDist, vy + Math.sin(cpAngle) * cpDist, vx + Math.cos(endAngle) * length, vy + Math.sin(endAngle) * length);
            ctx.stroke();
        }
    }
    ctx.globalCompositeOperation = 'source-over';
 ...
 */
export function drawBarnacles(ctx, x, y, clusterRadius, options = {}) {
    const { count = 8, seed = 123, palette = surfacePalettes.rocky, } = options;
    const perm = createPermutation(seed);
 ...
 */
export function drawScarring(ctx, x, y, length, options = {}) {
    const { angle = 0, width = 3, palette = surfacePalettes.fleshy, } = options;
