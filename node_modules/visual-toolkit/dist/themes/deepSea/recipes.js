...
import { material3D } from '../../core/gradients.js';
import { drawPanelLines, drawGrille, drawLight, drawLens, drawTether } from '../../core/shapes.js';
 ...
 * });
 */
export function drawROV(ctx, x, y, scale = 1, options = {}) {
    const { lightsOn = true, showTether = true, showArm = true, time = 0, rotation = 0, } = options;
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(rotation);
 ...
 * });
 */
export function drawLightCone(ctx, x, y, angle, length, options = {}) {
    const { spread = 0.5, startOpacity = 0.15, color = { r: 255, g: 250, b: 230 }, } = options;
    ctx.save();
    ctx.translate(x, y);
 ...
 * });
 */
export function drawJellyfish(ctx, x, y, options = {}) {
    const { bellWidth = 50, tentacleCount = 4, time = 0, glowIntensity = 0.4, } = options;
 ...
 * });
 */
export function drawSpecimen(ctx, x, y, options = {}) {
    const { width = 70, time = 0, illumination = 0, lookAt, } = options;
