 ...
 */
export function softGardenBackground(ctx, width, height) {
 ...
 */
export function terrariumBackground(ctx, width, height) {
 ...
 */
export function gardenWindowBackground(ctx, width, height) {
    // Base muted gradient
    const grad = ctx.createLinearGradient(0, 0, 0, height);
 ...
 */
export function overgrownBackground(ctx, width, height) {
    ...
    const radialGrad = ctx.createRadialGradient(width / 2, height / 2, Math.min(width, height) * 0.3, width / 2, height / 2, Math.max(width, height) * 0.8);
 ...
 */
export function applyDepthLayers(ctx, baseColor, width, height, layerCount = 3) {
 ...
 */
export function drawSunRays(ctx, width, height, rayCount = 4) {
    ctx.save();
    for (let i = 0; i < rayCount; i++) {
        ...
        ctx.lineTo(startX - Math.sin(angle) * (width * 0.15), startY + Math.cos(angle) * (height * 0.15));
        ctx.closePath();
        ctx.fill();
    }
    ctx.restore();
 ...
 */
export function applyFrostedGlassEffect(ctx, width, height, intensity = 0.1) {
 ...
 */
export function drawAtmosphericFog(ctx, width, height, baseAlpha = 0.3) {
    const grad = ctx.createLinearGradient(0, 0, 0, height);
