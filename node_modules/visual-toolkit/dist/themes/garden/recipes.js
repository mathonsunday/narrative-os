 ...
 */
export function drawSoftGardenWindow(ctx, x, y, width, height, options = {}) {
 ...
 */
export function drawTerrariumView(ctx, x, y, width, height, options = {}) {
    ...
    plants.drawPlantSilhouette(ctx, width * 0.15, height * 0.4, height * 0.5, colors.terrarium.background.shadow, 7);
    plants.drawPlantSilhouette(ctx, width * 0.85, height * 0.35, height * 0.55, colors.terrarium.background.shadow, 8);
    ctx.globalAlpha = 1;
 ...
 */
export function drawGardenThroughGlass(ctx, x, y, width, height, options = {}) {
    const { time = 0, intensity = 1.0 } = options;
    ctx.save();
    ctx.translate(x, y);
 ...
 */
export function drawOvergrownCamera(ctx, x, y, width, height, options = {}) {
    const { time = 0, growthLevel = 0, intensity = 1.0 } = options;
    ctx.save();
    ctx.translate(x, y);
    ...
    for (let i = 0; i < 3; i++) {
        ...
        plants.drawOrganicVine(ctx, -10, startY, 60 + i * 20, 0.2 + Math.random() * 0.3, 2 + i * 0.5, colors.overgrown.growth.vine, 0, 2);
    }
    // Right edge vines
    for (let i = 0; i < 3; i++) {
        const startY = (height / 3) * i + (Math.cos(time * TIMING.verySlow + i * 1.5) * 20);
        plants.drawOrganicVine(ctx, width + 10, startY, 60 + i * 20, Math.PI - (0.2 + Math.random() * 0.3), 2 + i * 0.5, colors.overgrown.growth.vine, 0, 2);
    }
    // Top vines
    for (let i = 0; i < 2; i++) {
        const startX = (width / 2.5) * i + (Math.sin(time * TIMING.verySlow + i * 2) * 20);
        plants.drawOrganicVine(ctx, startX, -10, 50 + i * 15, Math.PI / 2 + (Math.random() - 0.5) * 0.4, 1.5 + i * 0.5, colors.overgrown.growth.vine, 0, 2);
    }
    ctx.globalAlpha = 1;
