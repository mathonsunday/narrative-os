...
import { simplex2D, getOrCreatePermutation } from './textures.js';
/**
 * Draw growing roots that extend from an origin point
 */
export function drawGrowingRoots(ctx, options) {
    const { origin, direction = 'down', length, branches = 2, thickness = 3, color = '#3d2817', time = 0, seed = 0, } = options;
    ctx.save();
 ...
 */
export function drawVines(ctx, options) {
    const { origin, length, thickness = 2, color = '#2d5a3d', path, } = options;
    ctx.save();
    ctx.strokeStyle = color;
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
    ...
    if (path && path.length > 1) {
        ...
        const validPath = path.filter((p) => p != null &&
            typeof p === 'object' &&
            typeof p.x === 'number' &&
            typeof p.y === 'number' &&
            !isNaN(p.x) &&
        ...
        if (validPath.length < 2) {
            console.warn(`drawVines: Path array has insufficient valid points (${validPath.length} valid out of ${path.length} total). ` +
 ...
 */
export function drawGrowthIndicator(ctx, options) {
    const { x, y, width, height, growthLevel, style = 'vines', color = '#5a7a4d', } = options;
    ctx.save();
 ...
 */
export function drawOrganicVine(ctx, options) {
    const { origin, target, length, branchingDepth = 2, organicVariation = 0.5, hasLeaves = false, hasTendrils = false, growthLevel = 1, thickness = 4, color = '#2d5a3d', leafColor, time = 0, seed = 0, } = options;
    ...
    const leafPositions = [];
    // Draw main vine stem
    drawVineBranch(ctx, origin.x, origin.y, initialAngle, thickness, length * growthLevel, 0, // depth 0 = main stem
    branchingDepth, organicVariation, perm, seed, time, normalizedColor, target, branchPoints, leafPositions, hasLeaves, hasTendrils);
    // Recursively draw collected branch points
    for (const bp of branchPoints) {
        if (bp.thickness >= 1.5) {
            drawVineBranch(ctx, bp.x, bp.y, bp.angle, bp.thickness, length * growthLevel * (1 - bp.progress) * 0.6, 1, // depth 1 = first-level branch
            branchingDepth, organicVariation, perm, seed + bp.progress * 1000, time, normalizedColor, undefined, // branches don't have targets
            [], // sub-branches collected separately
            leafPositions, hasLeaves, hasTendrils);
        }
    }
 ...
 */
function drawVineBranch(ctx, startX, startY, startAngle, startThickness, totalLength, currentDepth, maxDepth, variation, perm, seed, time, color, target, branchPoints, leafPositions, hasLeaves, hasTendrils) {
    if (totalLength < 5 || startThickness < 0.5)
        return;
 ...
 */
function drawTendril(ctx, x, y, angle, thickness, perm, seed, color) {
 ...
 */
function drawSimpleLeaf(ctx, x, y, angle, size, color) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(angle);
 ...
 */
export function drawOrganicRoots(ctx, options) {
    const { origin, mainRootCount = 4, maxDepth = 3, length, growthLevel = 1, thickness = 5, color = '#3d2817', surfaceY, time = 0, seed = 0, } = options;
    const perm = getOrCreatePermutation(seed);
    ctx.save();
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
    ...
    for (let r = 0; r < rootCount; r++) {
        ...
        drawRootBranch(ctx, origin.x, origin.y, rootAngle, rootThickness, rootLength, 0, maxDepth, perm, rootSeed, time, color, surfaceY);
    }
    ctx.restore();
}
/**
 ...
 */
function drawRootBranch(ctx, startX, startY, startAngle, startThickness, totalLength, currentDepth, maxDepth, perm, seed, time, color, surfaceY) {
    if (totalLength < 8 || startThickness < 1 || currentDepth > maxDepth)
        return;
    ...
    for (let i = 0; i < segmentCount; i++) {
        ...
        if (currentDepth < maxDepth &&
            ...
            drawRootBranch(ctx, currentX, currentY, branchAngle, currentThickness * 0.5, totalLength * (1 - progress) * 0.5, currentDepth + 1, maxDepth, perm, segmentSeed + 1000, time, color, surfaceY);
        }
...
function normalizeColor(color) {
    if (!color)
        return '#2d5a3d'; // Default fallback
