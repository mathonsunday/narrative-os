 ...
 * Draw a single leaf
 */
export function drawLeaf(ctx, options) {
    const { x, y, size = 20, rotation = 0, type = 'simple', color = '#3d6a4d', opacity = 1, growthLevel = 1, seed = 0, } = options;
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(rotation);
 ...
 */
export function drawLeaves(ctx, leaves) {
    leaves.forEach((leaf) => {
 ...
 */
function drawSimpleLeaf(ctx, size, color) {
    ctx.fillStyle = color;
    ctx.beginPath();
    // Teardrop shape using Bezier curve
    ctx.moveTo(0, -size * 0.5); // Top point
    ctx.bezierCurveTo(size * 0.3, -size * 0.2, // Control point 1
    size * 0.4, size * 0.3, // Control point 2
    0, size * 0.5 // Bottom point
    );
    ctx.bezierCurveTo(-size * 0.4, size * 0.3, // Control point 1
 ...
 */
function drawVeinedLeaf(ctx, size, color) {
    // Draw leaf base
    drawSimpleLeaf(ctx, size, color);
    // Draw veins
 ...
 */
function drawOrganicLeaf(ctx, size, color) {
    ctx.fillStyle = color;
    ctx.beginPath();
 ...
 */
function drawDetailedLeaf(ctx, size, color, growthLevel, seed) {
 ...
 */
function drawLeafMottling(ctx, size, baseRgb, perm, seed, growthLevel) {
 ...
 */
function drawRecursiveVeins(ctx, size, color, perm, seed, growthLevel) {
    ...
    ctx.bezierCurveTo(simplex2D(seed, 0, perm) * 2, 0, simplex2D(seed + 100, 0, perm) * 2, size * 0.3, 0, size * 0.45);
    ctx.stroke();
    ...
    for (let i = 0; i < secondaryCount; i++) {
        ...
        for (const side of [-1, 1]) {
            ...
            ctx.beginPath();
            ctx.moveTo(0, y);
            ctx.quadraticCurveTo(side * widthAtY * 0.3 + simplex2D(seed + i * 20, side * 100, perm) * 3, y + size * 0.02, endX, endY);
            ctx.stroke();
 ...
 */
export function drawFlower(ctx, options) {
    const { x, y, petalCount = 5, petalLength = 15, color = '#e8a0b0', centerColor = '#f5d76e', growthStage = 1, rotation = 0, seed = 0, } = options;
    const perm = getOrCreatePermutation(seed);
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(rotation);
    ...
    for (let i = 0; i < actualPetalCount; i++) {
        ...
        const rgb = hexToRgb(color);
        const petalColor = rgbToHex(Math.min(255, Math.max(0, rgb.r + colorVar)), Math.min(255, Math.max(0, rgb.g + colorVar)), Math.min(255, Math.max(0, rgb.b + colorVar)));
        drawSinglePetal(ctx, baseOffset, petalLength * lengthVar * growthStage, petalLength * 0.35 * widthVar, openAngle, petalColor, perm, petalSeed);
        ctx.restore();
    }
    // Draw center disk
    if (growthStage > 0.3) {
 ...
 */
function drawSinglePetal(ctx, baseOffset, length, width, openAngle, color, perm, seed) {
    ctx.save();
    ...
    ctx.moveTo(0, 0);
    // Right side
    ctx.bezierCurveTo(width * 0.8 + simplex2D(seed, 0, perm) * width * 0.2, -length * 0.3, width * 0.6 + simplex2D(seed, 50, perm) * width * 0.2, -length * 0.7, simplex2D(seed, 100, perm) * width * 0.1, -length);
    // Left side (back to base)
    ctx.bezierCurveTo(-width * 0.6 - simplex2D(seed, 150, perm) * width * 0.2, -length * 0.7, -width * 0.8 - simplex2D(seed, 200, perm) * width * 0.2, -length * 0.3, 0, 0);
    ctx.closePath();
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.quadraticCurveTo(simplex2D(seed + 300, 0, perm) * width * 0.2, -length * 0.5, 0, -length * 0.85);
    ctx.stroke();
    ctx.restore();
}
/**
 ...
 */
function drawFlowerCenter(ctx, size, color, perm, seed) {
    const rgb = hexToRgb(color);
    // Base circle
    ctx.fillStyle = color;
    ctx.beginPath();
    ...
    for (let i = 0; i < dotCount; i++) {
        ...
        ctx.fillStyle = rgbToHex(Math.max(0, Math.min(255, rgb.r - colorShift)), Math.max(0, Math.min(255, rgb.g - colorShift)), Math.max(0, Math.min(255, rgb.b - colorShift)));
        ctx.beginPath();
