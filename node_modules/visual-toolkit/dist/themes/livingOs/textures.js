 ...
 */
export function getOrCreatePermutation(seed) {
 ...
 */
export function precomputePermutations(seeds) {
 ...
 */
export function clearPermutationCache() {
 ...
 */
export function drawBarkTexture(ctx, options) {
    const { width, height, grainFlow = 'vertical', age = 0.5, growthLevel = 0, knotDensity = 0.3, mossPatches = false, roughness = 0.5, color, woodType = 'oak', seed = 0, time = 0, } = options;
    ...
    if (width > MAX_WIDTH || height > MAX_HEIGHT) {
        console.warn(`drawBarkTexture: Dimensions ${width}x${height} exceed safe limit. ` +
    ...
    try {
        ...
        if (error instanceof RangeError || error instanceof DOMException) {
            ...
            console.error(`drawBarkTexture: Allocation failed. Falling back to ${fallbackWidth}x${fallbackHeight}.`);
            return drawBarkTexture(ctx, {
                ...options,
                width: fallbackWidth,
 ...
 */
export function drawBarkTextureCached(ctx, options) {
    const { cacheKey, cacheTolerance = DEFAULT_CACHE_TOLERANCE, growthLevel = 0, ...textureOpts } = options;
 ...
 */
export function drawOrganicSurface(ctx, options) {
 ...
 */
function createPaletteFromColor(hex) {
